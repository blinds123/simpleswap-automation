...
const MERCHANT_WALLET =
  process.env.MERCHANT_WALLET || "0x1372Ad41B513b9d6eC008086C03d69C635bAE578";
...
const PRICE_POINTS = (process.env.PRICE_POINTS || "19,29,39")
  .split(",")
...
async function loadPoolIntoMemory() {
  try {
    if (existsSync(POOL_FILE)) {
      ...
      memoryPool = pools;
      const summary = PRICE_POINTS.map(
        (p) => `$${p}: ${memoryPool[String(p)].length}`,
      ).join(", ");
...
async function createExchangeWithRetry(amountUSD, retries = MAX_RETRIES) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      console.log(
        `🔄 [CREATE] Attempt ${attempt}/${retries} for $${amountUSD}...`,
      );
...
async function createExchange(amountUSD, walletAddress = MERCHANT_WALLET) {
  ...
  try {
    ...
    await page.evaluate(() => {
      document
        .querySelectorAll(
          '[data-testid="info-message"], [role="alert"], .cookies-banner',
        )
...
async function replenishPool(priceKey) {
  ...
  try {
    ...
    if (needed <= 0) {
      console.log(
        `✅ [REPLENISH-${priceKey}] Pool full (${currentSize}/${config.size})`,
      );
      return;
    }

    console.log(
      `🔧 [REPLENISH-${priceKey}] Need ${needed} exchanges (${currentSize}/${config.size})`,
    );
    ...
    for (let i = 0; i < needed; i++) {
      try {
        ...
        isDirty = true;
        console.log(
          `📦 [REPLENISH-${priceKey}] ${memoryPool[priceKey].length}/${config.size}`,
        );
        ...
        console.error(
          `🚨 [REPLENISH-${priceKey}] Failed to create exchange: ${error.message}`,
        );
    ...
    console.log(
      `✅ [REPLENISH-${priceKey}] Complete: ${memoryPool[priceKey].length}/${config.size}`,
    );
  } finally {
...
async function healthCheck() {
  ...
  for (const price of PRICE_POINTS) {
    ...
    const status =
      current >= target ? "✅" : current >= MIN_POOL_SIZE ? "⚠️" : "🚨";
  ...
  console.log(
    `  📊 Stats: consumed=${stats.totalConsumed}, replenished=${stats.totalReplenished}, failed=${stats.failedReplenishments}\n`,
  );
}
...
function startSelfPing() {
  ...
  setInterval(async () => {
    try {
      ...
      console.log(
        `🏓 [SELF-PING] Failed (expected if local): ${error.message}`,
      );
    }
  }, SELF_PING_INTERVAL);
}
